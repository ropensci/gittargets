...
You will also need command line Git, available at <https://git-scm.com/downloads>. Please make sure Git is reachable from your system path environment variables. To control which Git executable `gittargets` uses, you may set the `TAR_GIT` environment variable with `usethis::edit_r_environ()` or `Sys.setenv()`. The following R code will run without error if Git is ready to use.
