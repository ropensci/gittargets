...
If your `targets` pipeline generates large data files, consider installing [`git-lfs`](https://git-lfs.github.com). This may not necessarily speed up the process of staging files, but commits should be much faster. Once you install [`git-lfs`](https://git-lfs.github.com), it should just work on your project right out of the box because `tar_git_init()` writes the following to `_targets/.gitattributes`:
