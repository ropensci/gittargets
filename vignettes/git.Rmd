...
If your `targets` pipeline generates large data files, consider installing [`git-lfs`](https://git-lfs.github.com). It should just work on your project right out of the box because `tar_git_init()` writes a `.gitattributes` file to the data store. This may not necessarily speed up the process of staging files, but commits should be much faster.
